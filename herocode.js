...
// Check for mobile to adjust settings
if (isMobile()) {
...
// Adjusted event listeners for hover functionality
canvas.addEventListener('mousemove', function (e) {
    var pointer = pointers[0];
    var posX = scaleByPixelRatio(e.offsetX);
    var posY = scaleByPixelRatio(e.offsetY);
    if (!pointer.down) {
        updatePointerDownData(pointer, -1, posX, posY); // Simulate pointer down
    }
...
function updatePointerDownData(pointer, id, posX, posY) {
    pointer.id = id;
    pointer.down = true; // Always true to simulate continuous effect
    pointer.moved = false;
